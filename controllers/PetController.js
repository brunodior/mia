...
module.exports = class PetController {
    ...
    static async schedule(req, res){
        ...
        // check if user registreded the pet
         const token = getToken(req)
         const user = await getUserByToken(token)
 
        //  check if pet is myself
        ...
        if(pet.adopter){
            if(pet.adopter._id.equals(user._id)){
                res.status(422).json({message: 'Você ja agendou uma viista para este Pet!'})
                return
            }
        }
        ...
        res.status(200).json({message: `A visita foi agendada com sucesso, entre em contato ${pet.user.name} pelo telefone ${pet.user.phone}`})
    }
